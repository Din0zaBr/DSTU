...
def translate_batch(texts: List[str], batch_size: int = 5) -> List[str]:
    ...
    for i in range(0, len(texts), batch_size):
        ...
        try:
            print(f"Переводим пакет из {len(batch)} сообщений...")
            translator = GoogleTranslator(source='auto', target='ru')
            batch_translations = translator.translate_batch(batch)
            print(f"Успешно переведено {len(batch_translations)} сообщений")
            translations.extend(batch_translations)
            # Добавляем задержку между пакетами
            time.sleep(1)
        except Exception as e:
            with print_lock:
                print(f"Ошибка при переводе пакета: {str(e)}")
                print("Пробуем перевести по одному сообщению...")
            
            # Пробуем перевести по одному сообщению
            for text in batch:
                try:
                    translator = GoogleTranslator(source='auto', target='ru')
                    translation = translator.translate(text)
                    translations.append(translation)
                    time.sleep(0.5)  # Задержка между отдельными переводами
                except Exception as e:
                    with print_lock:
                        print(f"Не удалось перевести сообщение: {text}")
...
def process_csv(input_file):
    ...
    try:
        ...
        num_workers = 2  # Уменьшаем количество параллельных потоков
