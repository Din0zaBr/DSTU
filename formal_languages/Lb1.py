...
def defense2(array):  # Проверка на корректность ввода
    ...
    Параметры:
        array (список): Список кортежей, каждый из которых содержит два элемента. Каждый элемент представляет собой строку, которая будет проверена на соответствие регулярному выражению.
...
def interface2(rule_number):
    ...
    Пользователь может вводить правила, заполняя значениями полей ввода.
    После того как пользователь ввел правила, он может нажать кнопку "Далее", чтобы продолжить.
    Если введенные правила являются некорректными, функция отображает дополнительное поле ввода, предлагая пользователю ввести правильные правила.
    """
    flag = True
    while True:
        if flag:
            layout = [[Input(), Text('->'), Input()] for i in range(0, rule_number * 2, 2)]
