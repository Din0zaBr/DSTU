...
def can_form_branches(
    ...
    for discomfort in discomforts:
        ...
        if members_in_current_branch < 1 and discomfort <= max_uncomfortable:
            count += 1  # Завершаем текущую бригаду, т.е. бригада сформирована
...
def find_minimum_max_uncomfortable(n: int, r: int, c: int, heights: Iterable[int]) -> int:
    ...
    Чтобы нам найти всех ближайших людей друг к другу, логичнее всё отсортировать.
    ...
    left = -1
    right = heights[-1] - heights[0]  # начальная верхняя граница (максимально возможная разница ростов)

    while left + 1 < right:
        mid = (left + right) // 2
        if can_form_branches(
                mid, discomforts, r,c):
            # Можно ли сформировать r бригад, при условии, что максимальная разница ростов не превышает mid
