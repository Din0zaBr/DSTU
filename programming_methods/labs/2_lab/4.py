...
def restore_tournament_table(num_teams: int, points: List[int]) -> Iterable[Iterable[int]]:
    ...
    # Элементы на главное диагонали останутся нулевыми (команда не играет с собой)
    for i in range(num_teams - 1):  # Проходит по каждой команде, кроме последней
        # print(*tournament_table, sep="\n")
        # Сортируем команды по очкам и получаем отсортированные списки очков и индексов.
        ...
        # print("-" * 20)
        for j in range(i + 1, num_teams):  # Обрабатывает пары команд
            if tournament_table[sorted_indices[i]][sorted_indices[j]] > 0:
                # Если результат матча уже заполнен, пропускаем эту пару.
                continue
            elif sorted_points[i] > 0:  # При наличии очков у команды - ничья (по 1 очку каждой)
                # Ничья
            ...
            elif sorted_points[i] == 0:  # При отсутствии очков у команды - поражение (2 очка противнику)
                # Поражение
